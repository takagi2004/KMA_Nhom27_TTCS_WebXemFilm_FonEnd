        ...
        {phimNoiBat.map((phim, index) => (
          <div key={phim.idPhim} className={`carousel-item ${index === 0 ? "active" : ""}`}>
            ...
            <div className="carousel-overlay-bottom-left text-start">
              <h1 className="fw-bold display-4">{phim.tenPhim}</h1>

              {/* Badge trạng thái */}
              <div className="mb-3">
                <span className="badge bg-success me-2 fs-6">Top Thịnh Hành</span>
                {phim.goiDangKy?.map((goi, i) => (
                  <span key={i} className="badge bg-info me-2 fs-6">{goi}</span>
                ))}
              </div>

              {/* Thông tin nhanh */}
              <p className="mb-2 fs-5">
                <span className="text-warning fw-bold me-3 fs-4">★ {phim.diemTrungBinh?.toFixed(1) || 0}</span>
                {phim.namPhatHanh} • {phim.soLuong} tập
              </p>

              {/* Thể loại */}
              <p className="mb-3 text-light fs-6">
                {phim.theLoai?.join(" • ")}
              </p>

              {/* Mô tả */}
              <p className="text-light fs-5" style={{ maxWidth: "600px" }}>
