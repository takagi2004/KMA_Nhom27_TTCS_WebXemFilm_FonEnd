...
const ChiTietPhim: React.FC = () => {
  ...
  const [soLuongDeXuat, setSoLuongDeXuat] = useState<number>(window.innerWidth < 768 ? 4 : 10);
  ...
  useEffect(() => {
    if (id) {
      ...
      PhimAPI.getTopThinhHanh(20).then((list) => {
  ...
  const handleXemThemDeXuat = () => {
    setSoLuongDeXuat((prev) => prev + 4);
  };
      ...
      <div className="row">
        ...
        <div className="col-lg-4">
          ...
          <div className="d-flex flex-column gap-3">
            {deXuat.slice(0, soLuongDeXuat).map((p) => (
              <Link
                key={p.idPhim}
                to={`/phim/${p.idPhim}`}
          ...
          {soLuongDeXuat < deXuat.length && (
            <button
              className="btn btn-sm btn-outline-light mt-3 w-100"
              onClick={handleXemThemDeXuat}
            >
              Xem thêm
            </button>
          )}
