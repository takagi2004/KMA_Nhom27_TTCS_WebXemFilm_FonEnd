...
const DanhSachPhim: React.FC<DanhSachPhimProps> = ({ title, fetchApi, slidesToShow = 8 }) => {
  ...
  const settings = {
    ...
    responsive: [
      ...
      {
        breakpoint: 768,
        settings: { slidesToShow: 2, slidesToScroll: 2, centerMode: true, centerPadding: "20px" },
      },
    ],
  };

  return (
    <section className="danh-sach-phim-wrapper">
      <h3 className="mb-4 text-light fw-bold ps-2 text-start">{title}</h3>
      <Slider {...settings}>
        {phimList.map((phim) => (
